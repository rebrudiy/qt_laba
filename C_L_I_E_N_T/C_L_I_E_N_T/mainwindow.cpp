...
void MainWindow::slotReadyRead(){
    ...
    if(in.status() == QDataStream::Ok){
        for(;;){
            ...
            QString status;
            in >> status;
            if(status == "message" || status == "new_user" || status == "new_file"){
                if(!automatic_reception){
                accept_file *e = new accept_file();
                e->show();
                connect(e,&accept_file::f,this,&MainWindow::slotformaccept);
                loop.exec();
                }else{
                    cond = 1;
                }
            ...
            if(status == "connected"){
               ...
               on_actionDisconnect_triggered();
            }else if(status == "new_image" && cond == 1){
                QPixmap pip;
                in >> pip;
                QString username1;
                in >> username1;
                ...
                QVector<QString>s;
                in >> s;
                s.push_back(user_informer);
                qDebug() << s[0] << " " << s[1];
                others_info *h = new others_info(nullptr,s);
                connect(h,&others_info::f,this,&MainWindow::g);
                h->show();
            }
            QThread::sleep(1);
            ui->progressBar->setValue(100);
...
void MainWindow::g(QString pth, QString ur_nm)
{
    pth += "^" + ur_nm;
    qDebug() << pth;
    sendToServer("current_file",pth);
}
...
{
    this->hide();
    show_history *s = new show_history(nullptr,ui->listWidget_2,history);
    s->show();
    connect(s,&show_history::again,this,&MainWindow::pokazat_okno);
}

void MainWindow::pokazat_okno()
{
    this->show();
}
